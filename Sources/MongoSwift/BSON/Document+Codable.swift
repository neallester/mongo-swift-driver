	...
	private static func recursivelyEncodeUnkeyed(_ value: BsonValue?, to container: inout UnkeyedEncodingContainer) throws {
		...
		// Otherwise get a keyed container and decode each key one by one
			...
			for key in container.allKeys {
				...
				output[k] = try decodeBsonValueFromKeyed(key: key, container: container)
			}
			self = output
		}
	}
...
/// Switch through all possible BSON types and try recursively decoding the value
/// stored under `key` as that type from the provided keyed container.
internal func decodeBsonValueFromKeyed<T>(key: T, container: KeyedDecodingContainer<T>) throws -> BsonValue {
	if let value = try? container.decode(Double.self, forKey: key) {
		...
		while !nested.isAtEnd {
			res.append(try decodeBsonValueFromUnkeyed(container: &nested))
		}
		return res
...
/// Switch through all possible BSON types and try recursively decoding the next value
/// as that type from the provided unkeyed container.
internal func decodeBsonValueFromUnkeyed(container: inout UnkeyedDecodingContainer) throws -> BsonValue {
